...
// Base OCPP Message Schema
export const OcppMessageSchema = z.tuple([
  z.number(), // Message Type (2=CALL, 3=CALLRESULT, 4=CALLERROR)
  z.string(), // Message ID
  z.string(), // Action (for CALL) or Error Code (for CALLERROR)
  z.any()     // Payload
]);
...
export const ChangeAvailabilityConfSchema = z.object({
...
export const ResetConfSchema = z.object({
  status: z.enum(['Accepted', 'Rejected'])
});
...
export const UnlockConnectorConfSchema = z.object({
...
export function validateByAction(action, payload, isRequest = true) {
  const schemas = {
    ...
    ChangeAvailability: {
      req: ChangeAvailabilityReqSchema,
      conf: ChangeAvailabilityConfSchema
    },
    Reset: {
      req: ResetReqSchema,
      conf: ResetConfSchema
    },
    UnlockConnector: {
