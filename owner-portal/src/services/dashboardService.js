...
export class DashboardService {
  ...
  static async getDashboardData(ownerId, timeRange = 'month', selectedMonth = null, selectedYear = null) {
    try {
      ...
      return this.calculateDashboardMetrics(stations, sessions, payments, timeRange, selectedMonth, selectedYear);
    } catch (error) {
  ...
  static calculateDashboardMetrics(stations, sessions, payments, timeRange, selectedMonth, selectedYear) {
    ...
    // Đếm stations hoạt động
    const activeStations = stations.filter(station => 
      station.status === 'online' || station.status === 'Online'
    ).length;
